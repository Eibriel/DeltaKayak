...
func _notification(what):
	if what == NOTIFICATION_PREDELETE:
		...
		if screen_shader.is_valid():
			rd.free_rid(screen_shader)
...
var screen_shader : RID
var screen_pipeline : RID
...
func _initialize_compute():
	...
	# Create our shader
	var shader_file = load("res://compositor/screen/screen.glsl")
	var shader_spirv: RDShaderSPIRV = shader_file.get_spirv()
	screen_shader = rd.shader_create_from_spirv(shader_spirv)
	screen_pipeline = rd.compute_pipeline_create(screen_shader)
	...
	if rd and p_effect_callback_type == CompositorEffect.EFFECT_CALLBACK_TYPE_POST_TRANSPARENT:
		...
		if render_scene_buffers:
			...
			for view in range(view_count):
				...
				var input_set = UniformSetCacheRD.get_cache(screen_shader, 0, [ uniform ])
